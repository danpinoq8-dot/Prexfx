  ...
  return (
    <div className="flex flex-col h-[calc(100vh-56px)] md:h-[calc(100vh-52px)]">
      <div className="p-4 border-b border-border">
        <h2 className="text-[10px] uppercase tracking-[0.2em] text-muted-foreground">Prexi AI Terminal — Cerebras Brain</h2>
        <p className="text-[9px] text-muted-foreground mt-1">Ask about market sentiment, strategy, or trade analysis</p>
      </div>
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {messages.length === 0 && (
          <div className="text-center mt-12 space-y-3">
            ...
            <div className="space-y-2">
              {["Summarize the NFP impact for today", "What's the best XAU/USD entry right now?", "Analyze GBP/JPY sentiment"].map((q) => (
        ...
        {messages.map((msg, i) => (
          <div key={i} className={`max-w-[85%] p-3 rounded-xl text-xs leading-relaxed ${msg.role === "assistant" ? "glass-panel mr-auto" : "bg-accent ml-auto text-accent-foreground"}`}>
            {msg.role === "assistant" ? (
      ...
      <div className="p-4 border-t border-border glass-panel">
        <div className="flex items-center gap-2">
          <input type="text" value={input} onChange={(e) => setInput(e.target.value)}
            ...
            disabled={isLoading} />
          <button className="p-3 rounded-xl bg-accent text-accent-foreground hover:bg-accent/80 transition-colors"><Mic size={16} /></button>
